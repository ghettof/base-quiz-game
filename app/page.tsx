...
type Score = { username: string; category: string; score: number; total: number; };
    ...
    return (
      <main className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6">
        <div className="bg-zinc-900 rounded-2xl p-8 max-w-md w-full shadow-xl">
          <h1 className="text-2xl font-bold mb-6 text-center">🏆 Liderlik Tablosu</h1>
          <div className="flex flex-col gap-2 mb-6">
            {leaderboard.map((s, i) => (
              <div key={i} className="flex justify-between items-center bg-zinc-800 rounded-xl px-4 py-3">
                <div className="flex items-center gap-3">
                  <span className="text-zinc-400 text-sm w-5">{i + 1}.</span>
                  <div>
                    <p className="font-bold">{s.username}</p>
                    <p className="text-zinc-400 text-xs">{categories.find(c => c.id === s.category)?.name}</p>
                  </div>
                </div>
                <span className="text-blue-400 font-bold">{s.score}/{s.total}</span>
              </div>
            ))}
          </div>
          <button onClick={restart} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition">
            Ana Menü
          </button>
        </div>
      </main>
    ...
    return (
      <main className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6">
        <div className="max-w-md w-full">
          ...
          <p className="text-zinc-400 text-center mb-8">Bir kategori seç ve başla!</p>
          ...
          <button onClick={() => { fetchLeaderboard(); setScreen("leaderboard"); }}
            className="w-full bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white font-bold py-3 rounded-xl transition">
            🏆 Liderlik Tablosu
          </button>
        </div>
      </main>
    ...
    return (
      <main className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6">
        <div className="bg-zinc-900 rounded-2xl p-8 max-w-md w-full text-center shadow-xl">
          ...
          <p className="text-5xl font-bold text-blue-400 mb-2">{score} / {qs.length}</p>
          ...
          <div className="mb-4">
            <input
              type="text"
              placeholder="Farcaster kullanıcı adın (örn: ghetoof.eth)"
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-white placeholder-zinc-500 mb-3"
            />
            <button onClick={saveScore} disabled={saving || !username.trim()}
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition mb-3">
